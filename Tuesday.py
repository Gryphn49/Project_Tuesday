...
import wolframalpha as w@  #Adds wolfram alpha
...
def sinput():
    if VOICEINPUT
        ...
        with sr.Microphone() as source:
            try:
                print(random.choice(begin_db))
            except KeyboardInterrupt:
                print("Exiting...")
                exit()
            uinput = r.listen(source)
    else:
        uinput = input(random.choice(begin_db))
    ...
    if not wm:
        ...
        if wm:
            ...
            try:
                ...
                debugging("No responce found")
                print("")
                wm = False # Look at in more detail later.
    ...
    try:
        ...
        if wolfA == True:
            try:
                ...
                client = w@.Client(app_id)  #connecting to info
